...
export default function Alert({text}: { text: string }) {
    ...
    return (
        <div className={"fixed top-0 right-0 m-10 flex justify-between"}>
            {show && (<div className="px-5 bg-yellow-400 text-white rounded-2xl font-mono font-bold"><p className={"my-6"}>{text}</p>
            </div>)}
            <button className="m-4 motion-safe:animate-pulse hover:animate-none" onMouseLeave={() => setShow(false)} onMouseOver={() => setShow(true)}>
