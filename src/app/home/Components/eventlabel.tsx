export default function EventLabel({name, href, color}: { name: string, href: string, color: string }) {
    return (<a href={href} target="_blank" className={"relative group cursor-pointer"}>
            ...
            <div className={`relative w-full h-16 rounded-2xl grid place-content-center font-mono ${color}`}>
                <h3
                   className={"text-white text-2xl group-hover:text-gray-100 transition duration-200"}>{name}</h3>
            </div>
        </a>
    )
}